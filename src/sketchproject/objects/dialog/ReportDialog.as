package sketchproject.objects.dialog
{
	...
	public class ReportDialog extends DialogOverlay
	{
		...
		/**
		 * Get instance of singleton pattern.
		 *
		 * @return
		 */
		...
		{
			if (myShop != null && competitor1Shop != null && competitor2Shop != null)
			{
				...
				// temporary count of market selected
				a = 0;
				b = 0;
				c = 0;
				...
				// push graph and set last graph y position from first data				
				...
				// build new graph each update from last y position
				for (var i:uint = 0, l:uint = myShop.listshare.length; i < l; i++)
				{
					// market share graph -----------------------------------------
					// my shop market share graph
